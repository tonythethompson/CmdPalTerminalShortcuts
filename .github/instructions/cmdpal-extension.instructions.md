...
Quick-action icon buttons shown at the trailing edge of list rows on hover or keyboard selection. Requires a CmdPal build and `Microsoft.CommandPalette.Extensions` SDK that includes the hover-action interfaces (not yet in all published NuGet versions — see **SDK version** below).
...
Set on items in `ListItem.MoreCommands` or a home `CommandItem.MoreCommands`:
...
Use negative `HoverOrder` to pin actions to the left of the strip (e.g. move-up/down before edit).

### Home row overrides (`ICommandItem2`)

For each top-level command returned from `TopLevelCommands()`:
...
public partial class MyCommandsProvider : CommandProvider
{
#if CMDPAL_HOVER_ACTIONS // see SDK version below
...
When unset, silent extensions keep legacy behavior: first three visible context commands on list pages; on home, host pin/dock commands are excluded from hover.

### SDK version and local builds

Hover APIs ship in newer `Microsoft.CommandPalette.Extensions` / Toolkit builds (PowerToys `feat/cmdpal-list-hover-actions` and later). Published NuGet **0.11** does not include them yet.

For CI or machines without a local PowerToys checkout:

- Gate hover code behind `#if CMDPAL_HOVER_ACTIONS` (defined when `UseLocalCmdPalSdk=true` in `Directory.Build.props`).
- NuGet-only builds compile without hover configuration; hover appears only when built against the local SDK and run on a CmdPal build with **List hover actions** enabled.

After deploying, run **Reload Command Palette Extension** in CmdPal.

### Agent checklist

- Do not assume hover works on every user's CmdPal — check the user setting and SDK version.
- Prefer `Explicit` + `ShowInHoverActions` when you need specific actions (edit, delete, pin) rather than relying on first-N order.
